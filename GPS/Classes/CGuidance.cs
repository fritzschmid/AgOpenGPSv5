    ...
    public class CGuidance
    {
        ...
        private void DoSteerAngleCalc()
        {
            ...
            if (mf.pn.speed > mf.startSpeed
                ...
                && Math.Abs(derivativeDistError) < 3)
            {
                if(mf.SteerMode==0)//Original
                {
                    //if over the line heading wrong way, rapidly decrease integral
                    if ((inty < 0 && distanceFromCurrentLineSteer < 0) || (inty > 0 && distanceFromCurrentLineSteer > 0))
                    {
                        inty += pivotDistanceError * mf.vehicle.stanleyIntegralGainAB * -0.05;
                    }
                    else
                    {
                        inty += pivotDistanceError * mf.vehicle.stanleyIntegralGainAB * -0.02;
                    }
                }
                if (mf.SteerMode == 1) // FRITZ
                {
                    //if over the line heading wrong way, rapidly decrease integral
                    ...
                    {
                        // grosse abweichung
                        inty += pivotDistanceError * mf.vehicle.stanleyIntegralGainAB * -0.9;
                    }
                    else
                    {
                        // kleine abweichnung
                        inty += pivotDistanceError * mf.vehicle.stanleyIntegralGainAB * -0.3;
                    }
                    // wert begrenzen
                    if (inty < -4) inty = -4;
                    if (inty > 4) inty = 4;
        ...
        public void StanleyGuidanceABLine0(vec3 curPtA, vec3 curPtB, vec3 pivot, vec3 steer)// ORIGINAL
        {
        ...
        public void StanleyGuidanceABLine1(vec3 curPtA, vec3 curPtB, vec3 pivot, vec3 steer)// FRITZ
        { //*********************************************************************************************************************************************************************************************
            //get the pivot distance from currently active AB segment   ///////////  Pivot  ////////////
            double dx = curPtB.easting - curPtA.easting;
            double dy = curPtB.northing - curPtA.northing;
            ...
            //how far from current AB Line is fix
            distanceFromCurrentLinePivot = ((dy * pivot.easting) - (dx * pivot.northing) + (curPtB.easting * curPtA.northing) - (curPtB.northing * curPtA.easting)) / Math.Sqrt((dy * dy) + (dx * dx));
            //distanceFromCurrentLinePivot = ((dy * steer.easting) - (dx * steer.northing) + (curPtB.easting * curPtA.northing) - (curPtB.northing * curPtA.easting)) / Math.Sqrt((dy * dy) + (dx * dx));
            
            if (!mf.ABLine.isHeadingSameWay)
                distanceFromCurrentLinePivot *= -1.0;

            mf.ABLine.distanceFromCurrentLinePivot = distanceFromCurrentLinePivot;
            double U = (((steer.easting - curPtA.easting) * dx) + ((steer.northing - curPtA.northing) * dy)) / ((dx * dx) + (dy * dy));

            rEastSteer = curPtA.easting + (U * dx);
            rNorthSteer = curPtA.northing + (U * dy);

            mf.ABLine.rEastAB = rEastSteer;
            mf.ABLine.rNorthAB = rNorthSteer;
            ...
            //how far from current AB Line is fix
            distanceFromCurrentLineSteer = ((dy * steer.easting) - (dx * steer.northing) + (curPtB.easting * curPtA.northing) - (curPtB.northing * curPtA.easting)) / Math.Sqrt((dy * dy) + (dx * dx));

            if (!mf.ABLine.isHeadingSameWay)
                distanceFromCurrentLineSteer *= -1.0;

            // calc point on ABLine closest to current position - for display only
            //U = (((steer.easting - steerA.easting) * dx) + ((steer.northing - steerA.northing) * dy)) / ((dx * dx) + (dy * dy));

            rEastSteer = steer.easting + (U * dx);
            rNorthSteer = steer.northing + (U * dy);
        ...
        public void StanleyGuidanceCurve0(vec3 pivot, vec3 steer, ref List<vec3> curList) //Original
        ...
        //***************************************************************************************************************************************************************************************************
        public void StanleyGuidanceCurve1(vec3 pivot, vec3 steer, ref List<vec3> curList) // Fritz
        {            //calculate required steer angle
