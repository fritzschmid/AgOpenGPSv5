...
namespace AgOpenGPS
{
    public class CABLine
    {
        ...
        public void BuildCurrentABLineList(vec3 pivot)
        {
            ...
            if(mf.ToolOverlapping ==false)
            {
                widthMinusOverlap = mf.tool.toolWidth;
            }
        ...
        public void GetCurrentABLine(vec3 pivot, vec3 steer)
        {
            ...
            //Stanley
            else if ((mf.SteerMode == 0) && (!mf.isReverse)) //Original
                mf.gyd.StanleyGuidanceABLine0(currentABLineP1, currentABLineP2, pivot, steer);
            else if ((mf.SteerMode == 1) && (!mf.isReverse)) //Fritz
                mf.gyd.StanleyGuidanceABLine1(currentABLineP1, currentABLineP2, pivot, steer);

            //Pure Pursuit
            else if ((mf.SteerMode == 2) || (mf.isReverse))//ORIGINAL rueckwaerts nur in pp mode
                ...
                //************************************************************************************************************************************************************
            {
                ...
                //how far from current AB Line is fix
                //distanceFromCurrentLinePivot = ((dy * pivot.easting) - (dx * pivot.northing) + (currentABLineP2.easting * currentABLineP1.northing) - (currentABLineP2.northing * currentABLineP1.easting)) / Math.Sqrt((dy * dy) + (dx * dx));
                distanceFromCurrentLinePivot = ((dy * steer.easting) - (dx * steer.northing) + (currentABLineP2.easting * currentABLineP1.northing) - (currentABLineP2.northing * currentABLineP1.easting)) / Math.Sqrt((dy * dy) + (dx * dx));        
...
*/
                ...
                ppRadiusAB = goalPointDistanceDSquared / (2 * (((goalPointAB.easting - steer.easting) * Math.Cos(localHeading))
                         + ((goalPointAB.northing - steer.northing) * Math.Sin(localHeading))));

                steerAngleAB = glm.toDegrees(Math.Atan(2 * (((goalPointAB.easting - steer.easting) * Math.Cos(localHeading))
                        + ((goalPointAB.northing - steer.northing) * Math.Sin(localHeading))) * mf.vehicle.wheelbase
                        / goalPointDistanceDSquared));
        ...
        public void DrawABLines()
        {
            ...
            if (mf.SteerMode>1 && mf.camera.camSetDistance > -200)
            {
        ...
        public void BuildTram()
        {
            ...
            for (int i = cntr; i < mf.tram.passes; i++)
            {
                ...
                for (int j = 0; j < tramRef.Count; j++)
                {
                    ...
                    P1.easting = (hsin * ((mf.tram.tramWidth * (pass + (i - 0.5))) - mf.tram.halfWheelTrack )) + tramRef[j].easting;
                    P1.northing = (hcos * ((mf.tram.tramWidth * (pass + (i - 0.5))) - mf.tram.halfWheelTrack )) + tramRef[j].northing;
