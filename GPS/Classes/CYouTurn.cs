    ...
    public class CYouTurn
    {
        ...
        public bool BuildABLineDubinsYouTurn(bool isTurnRight)
        {
            ...
            if (youTurnPhase == 0)
            {
                ...
                double toolTurnWidth = (mf.tool.toolWidth - mf.tool.toolOverlap) * rowSkipsWidth;
                double vehicleTurnRadius = toolTurnWidth / 2;
                ...
                if (mf.vehicle.minTurningRadius * 2 < toolTurnWidth)
                {
                    if (boundaryAngleOffPerpendicular < 0)
                    {
                        ...
                        if (isYouTurnRight)
                            ...
                            {
                                distanceTurnBeforeLine += (vehicleTurnRadius * Math.Tan(tangencyAngle));//short
                            }
                            else
                            {
                            ...
                            if (Properties.Settings.Default.setUturnAutoRadius)
                        {
                             distanceTurnBeforeLine += (vehicleTurnRadius / Math.Tan(tangencyAngle)); //long
                        }
                        else
                        {
                ...
                if (Properties.Settings.Default.setUturnAutoRadius & (mf.vehicle.minTurningRadius< vehicleTurnRadius)) CDubins.turningRadius = vehicleTurnRadius;
