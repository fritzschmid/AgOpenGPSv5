    ...
    public class CABCurve
    {
        ...
        public void GetCurrentCurveLine(vec3 pivot, vec3 steer)
        {
            ...
            if (ptCount > 0)
            {
                ...
                else if ((mf.SteerMode == 0) && (!mf.isReverse))//Stanley Original
                {
                    mf.gyd.StanleyGuidanceCurve0(pivot, steer, ref curList);
                }
                else if ((mf.SteerMode==1) && (!mf.isReverse))//Stanley Fritz
                {
                    mf.gyd.StanleyGuidanceCurve1(pivot, steer, ref curList);
                }
                else if ((mf.SteerMode==2) || (mf.isReverse))// Pure Pursuit Original------------------------------------------
                {
                ...
                { //SteerMode==3 PP Fritz ******************************************************************************************************************************************
                    ...
                    if (mf.vehicle.purePursuitIntegralGain != 0 && !mf.isReverse)
                    {
                        ...
                        if (mf.isAutoSteerBtnOn && mf.avgSpeed > 2.5 && Math.Abs(pivotDerivative) < 0.1)
                        {
                            ...
                            {
                                if (Math.Abs(distanceFromCurrentLinePivot) > 0.025)
                                {
                                    inty += pivotDistanceError * mf.vehicle.purePursuitIntegralGain * -0.02;
                                    if (inty > 0.2) inty = 0.2;
                                    else if (inty < -0.2) inty = -0.2;
                    ...
                    ppRadiusCu = goalPointDistanceSquared / (2 * (((goalPointCu.easting - steer.easting) * Math.Cos(localHeading)) + ((goalPointCu.northing - steer.northing) * Math.Sin(localHeading))));
                    steerAngleCu = glm.toDegrees(Math.Atan(2 * (((goalPointCu.easting - steer.easting) * Math.Cos(localHeading))
                   + ((goalPointCu.northing - steer.northing) * Math.Sin(localHeading))) * mf.vehicle.wheelbase / goalPointDistanceSquared));


                    if (mf.ahrs.imuRoll != 88888)
                        steerAngleCu += mf.ahrs.imuRoll * -mf.gyd.sideHillCompFactor;

                    //steerAngleCu += inty;//Fritz
        ...
        public void DrawCurve()
        {
            ...
            else //normal. Smoothing window is not open.
            {
                ...
                if (ptCount > 0 && isCurveSet)
                {
                    ...
                    if (mf.isPureDisplayOn && mf.SteerMode>1)
                    {
        ...
        public void BuildTram()
        {
            ...
            for (int i = cntr; i <= mf.tram.passes; i++)
            {
               ...
               //         * (mf.tram.tramWidth * (i + 0.5) - mf.tram.halfWheelTrack + mf.tool.halfToolWidth) * 0.999999;
                double distSqAway = (mf.tram.tramWidth * (i) - mf.tram.halfWheelTrack) * (mf.tram.tramWidth * (i) - mf.tram.halfWheelTrack) * 0.999999;
                ...
                for (int j = 0; j < refCount; j += 1)
                {
                    ...
                    (Math.Sin(glm.PIBy2 + refList[j].heading) *
                        ((mf.tram.tramWidth * (pass + i)) - mf.tram.halfWheelTrack)) + refList[j].easting,
                    (Math.Cos(glm.PIBy2 + refList[j].heading) *
                        ((mf.tram.tramWidth * (pass + i)) - mf.tram.halfWheelTrack)) + refList[j].northing);
