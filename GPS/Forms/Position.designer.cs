...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        public double guidanceLookAheadTime = 3;
        ...
        public double refImuHeading = 0.0;
        public int headingOK = 0;
        ...
        //Fritz
        int SteerGrenzeModus = 0;
        public bool ToolOverlapping = true;
        ...
        public void UpdateFixPosition()
        {
                ...
                case "Fix":
                    ...
                    /*
                    {
                        ...
                        if (Math.Abs(avgSpeed) > startSpeed)
                        {
                            ...
                            // Reverse detection mit imu

                            if (ahrs.isReverseOn & (Math.Abs(avgSpeed) < 9)) // speed zusaetzlich
                            {
                        ...
                        if (ahrs.imuHeading != 99999)
                        {
                            ...
                            if (!isReverse) imuGPS_Offset += (gyroDelta * (ahrs.fusionWeight));
                            else imuGPS_Offset += (gyroDelta * (0.005));
                    ...
                    }*/
                    {
                        ...
                        if (Math.Abs(avgSpeed) > startSpeed)
                        {
                            ...
                            //Fritz IMU ofset merken
                            //Fritz IMU ofset merken
                            //Difference between the IMU heading and the GPS heading
                            if ((headingOK < 1) && (ahrs.imuHeading != 99999))
                            {
                                headingOK = 1;
                                testDelta = imuHeading - gpsHeading;// - imuHeading;
                                if (testDelta < 0) testDelta += glm.twoPI;
                                else if (testDelta > glm.twoPI) testDelta -= glm.twoPI;
                                refImuHeading = testDelta;
                            }

                            // Reverse detection mit imu

                            if (ahrs.isReverseOn && (Math.Abs(avgSpeed)<9) && (headingOK>0)) // speed zusaetzlich
                            {

                                //what is angle between the last valid heading before stopping and one just now
                                //double delta = Math.Abs(Math.PI - Math.Abs(Math.Abs(newHeading - gpsHeading) - Math.PI));
                                double delta = Math.Abs(Math.PI - Math.Abs(Math.Abs(newHeading - (imuHeading-refImuHeading)) - Math.PI));
                                //delta=0; // nicht verwenden...
                                //ie change in direction
                                if (delta > 1.6) //1.57
                                {
                                    isReverse = true;
                        ...
                        else
                        {
                            ...
                            if (distanceCurrentStepFix > minFixStepDist)
                            {
                                ...
                                if (ahrs.isReverseOn && (headingOK>0)) // speed zusaetzlich  & (Math.Abs(avgSpeed) > 9)
                                {
                                    //what is angle between the last valid heading before stopping and one just now
                                    double delta = Math.Abs(Math.PI - Math.Abs(Math.Abs(newHeading - (imuHeading - refImuHeading)) - Math.PI));
                                    //double delta = Math.Abs(Math.PI - Math.Abs(Math.Abs(newHeading - gpsHeading) - Math.PI));
                                    //delta = 0; // verwenden wenn nix geschehen soll
                                    //ie change in direction
                                    {
                                        if (delta > 1.6 ) //1.57 
                                        {
                        ...
                        {
                            //current gyro angle in radians
                            //double imuHeading = (glm.toRadians(ahrs.imuHeading)); wird oben gelesen
                            ...
                            //ergaenzen des  refImuHeading's ofsets
                            testDelta = imuHeading - gpsHeading;
                            if (testDelta < 0) testDelta += glm.twoPI;
                            else if (testDelta > glm.twoPI) testDelta -= glm.twoPI;
                            
                            refImuHeading = refImuHeading*0.97 + testDelta * 0.03; // langsam nachlernen
                            if (refImuHeading < 0) refImuHeading += glm.twoPI;
                            else if (refImuHeading > glm.twoPI) refImuHeading -= glm.twoPI;
                ...
                case "VTG":
                    {
                        ...
                        if (ahrs.imuHeading != 99999)
                        {
                            ...
                            if (Math.Abs(gyroDelta) < 0.18)
                            {
                                ...
                                imuGPS_Offset += (gyroDelta * (0.02));//0.1
                                if (imuGPS_Offset > glm.twoPI) imuGPS_Offset -= glm.twoPI;
                                if (imuGPS_Offset < -glm.twoPI) imuGPS_Offset += glm.twoPI;
                            }
                            else
            ...
            if (!vehicle.ast.isInFreeDriveMode)
            {
                ...
                else
                {
                    ...
                    bool drinnenBnd = bnd.IsPointInsideFenceArea(steerAxlePos);
                    bool drinnenHeadland = bnd.IsPointInsideHeadArea(steerAxlePos);
                    if (drinnenBnd)
                    {
                        p_254.pgn[p_254.status] = 7; // alles drinnen setzen
                        if (!drinnenHeadland && SteerGrenzeModus==1)
                        {
                            p_254.pgn[p_254.status] = 3;// wieder wegnehmen
                            guidanceLineDistanceOff = 32020;// teste erstmal in v03
                        }
                    }
                    if (SteerGrenzeModus == 2)
                    {
                        p_254.pgn[p_254.status] = 7; // aktivieren
                    }
                }
