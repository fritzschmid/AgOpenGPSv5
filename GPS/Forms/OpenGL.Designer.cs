...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        public bool Messpunkt = false;
        public bool PunktFSsichtbar = false;
        public int PositionsNummer = 0;
        ...
        private void oglMain_Paint(object sender, PaintEventArgs e)
        {
            ...
            else
            {
                if (isGPSPositionInitialized)
                {
                    ...
                    // Fritzpunkte sichtbar machen
                    //
                    if ((PunktFSsichtbar) & (cPunktFs.punktArr.Count > 0)) DrawPointFS();
        ...
        private void DrawPointFS()
        {
            //********************************************************************************************
            // Fritz BoundariePoints
            int fspCnt = cPunktFs.punktArr.Count;
            if (fspCnt > 0)
            {
                for (int i = 0; i < fspCnt; i++)
                {
                    if (cPunktFs.punktArr[i].Art == 1)// Aktive feldpunkte
                    { 
                        GL.PointSize(4.0f);
                        GL.Color3((byte)200, (byte)200, (byte)40);
                        GL.Begin(PrimitiveType.Points);
                        GL.Vertex3(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, 0);
                        GL.End();
                        font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "&" + cPunktFs.punktArr[i].Nr.ToString());
                        //font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "-" + cPunktFs.punktArr[i].Nr.ToString());
                    }
                    else if(cPunktFs.punktArr[i].Art == 4)// Brache feldpunkte
                    {
                        GL.PointSize(4.0f);
                        GL.Color3((byte)40, (byte)255, (byte)40);
                        GL.Begin(PrimitiveType.Points);
                        GL.Vertex3(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, 0);
                        GL.End();
                        font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "&" + cPunktFs.punktArr[i].Nr.ToString());
                    ...
                    {
                        GL.PointSize(4.0f);
                        GL.Color3((byte)240, (byte)0, (byte)140);
                        GL.Begin(PrimitiveType.Points);
                        GL.Vertex3(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, 0);
                        GL.End();
                        font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "&" + cPunktFs.punktArr[i].Nr.ToString());
        ...
        private void DrawAntennePoint()
        {
            // Fritz AntennenAlsPunkt

            GL.PointSize(4.0f);
            GL.Color3((byte)0, (byte)95, (byte)255);
            GL.Begin(PrimitiveType.Points);
            GL.Vertex3(pn.fix.easting, pn.fix.northing, 0);
            GL.End();
            //font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "&" + cPunktFs.punktArr[i].Nr.ToString());
            //font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "-" + cPunktFs.punktArr[i].Nr.ToString());
            ...
            if (curve.isBtnCurveOn && !ct.isContourBtnOn)
            {
                ...
                if (curve.howManyPathsAway == 0)
                    font.DrawTextVehicle(-1, 2, "0", 1);
                else if (curve.howManyPathsAway > 0) 
                    font.DrawTextVehicle(-1, 2, curve.howManyPathsAway.ToString() + "R", 1.4);
                else 
                    font.DrawTextVehicle(-1, 2, curve.howManyPathsAway.ToString() + "L", 1.4);
            }
            else if (ABLine.isBtnABLineOn && !ct.isContourBtnOn)
            {
                ...
                if (ABLine.howManyPathsAway == 0)
                    ...
                    font.DrawTextVehicle(-1, 2,ABLine.howManyPathsAway.ToString() + "R", 1.4);
                else
                    font.DrawTextVehicle(-1, 2, ABLine.howManyPathsAway.ToString() + "L", 1.4);
            }
            //********************************************************************************************
        ...
        private void DrawLightBarText()
        {
            ...
            if (ct.isContourBtnOn || ABLine.isBtnABLineOn || curve.isBtnCurveOn)
            {
                ...
                //if (guidanceLineDistanceOff != 32000 && guidanceLineDistanceOff != 32020)
                {
                    ...
                    font.DrawText(center, 30, hede, 1.8);
                }
            }
        }
        ...
        private void DrawCompassText()
        {
            ...
            font.DrawText(center+10, 25, "F:" + (fixHeading * 57.2957795).ToString("N1"), 1);

            if (ahrs.imuHeading != 99999)
            {
                GL.Color3(0.98f, 0.72f, 0.3f);
                font.DrawText(center, 50, "G:" + (gpsHeading * 57.2957795).ToString("N1"), 1);

                if (!isSuperSlow) GL.Color3(0.9752f, 0.952f, 0.03f);
                else GL.Color3(0.298f, 0.972f, 0.99903f);

                font.DrawText(center, 70, "I:" + Math.Round(ahrs.imuHeading, 1).ToString(), 1);
                if (ahrs.isReverseOn)
                {
                    font.DrawText(center, 90, "r:" + Math.Round(refImuHeading * 57.2957795, 1).ToString(), 1);
                }
            }
