...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        public double HowMannyPartsAway;
        ...
        private void tmrWatchdog_tick(object sender, EventArgs e)
        {
            ...
            if (displayUpdateThreeSecondCounter != threeSeconds)
            {
                ...
                if (ToolOverlapping)
                {
                    lblTopData.Text = ((tool.toolWidth-tool.toolOverlap) * m2FtOrM).ToString("N2") + unitsFtM + " / " + (tool.toolWidth * m2FtOrM).ToString("N2") + unitsFtM + " -> " + vehicleFileName;
                }
                else
                {
                ...
                TramCount = Properties.Settings.Default.setTram_Counter;
            ...
            if (displayUpdateOneSecondCounter != oneSecond)
            {
                ...
                if (ABLine.isBtnABLineOn && !ct.isContourBtnOn)
                {
                    ...
                    HowMannyPartsAway = ABLine.howManyPathsAway;
                }
                if (curve.isBtnCurveOn && !ct.isContourBtnOn)
                {
                    btnEditAB.Text = ((int)(curve.moveDistance * 100)).ToString();
                    HowMannyPartsAway = curve.howManyPathsAway;
                }

                if (TramCount > 0)
                {
                    if ((HowMannyPartsAway % TramCount) == 0)
                    {
                        btnTramDisplayMode.BackColor = Color.LimeGreen;
                    }
                    else
                    {
                        btnTramDisplayMode.BackColor = Color.Tomato;
                    }
                }
                else
                {
                    btnTramDisplayMode.BackColor = Color.Transparent;
                }
                ...
                if (ToolOverlapping)
                {
                    lblTopData.BackColor = Color.DarkGray;
                }
                else
                {
                    lblTopData.BackColor = Color.Violet;
                }
            }
        ...
        {            //metric settings
            ...
            btnChangeMappingColor.Text = "v5.4 06";//Application.ProductVersion.ToString(CultureInfo.InvariantCulture);
            ...
            SteerMode = Properties.Settings.Default.setSteerMode;
            if (SteerMode<2)
            {
            ...
            btnStanleyPure.Text = SteerMode.ToString();
        ...
        public void SwapDayNightMode()
        {
            ...
            lblTopData.ForeColor = Color.Black;
            btnSteerHeadland1.ForeColor = Color.Black;
            btnStanleyPure.ForeColor = Color.DarkBlue;
        ...
        private void oglMain_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (point.X > centerLeft - 40 && point.X < centerLeft + 40 && point.Y > centerUp - 60 && point.Y < centerUp + 60)
                {
                    ...
                    headingOK = 0;// fuer kompasabgleich bei reverse direction den merker fuer abgleich neu laden
                    return;
                }
