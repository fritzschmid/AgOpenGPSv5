...
namespace AgOpenGPS
{
    public partial class FormABDraw : Form
    {
        ...
        private void FormABDraw_Load(object sender, EventArgs e)
        {
            ...
            nudDistance.Value = (decimal)Math.Round(((mf.tool.toolWidth * mf.m2InchOrCm) * 1), 0); // 
        ...
        {            //count the points from the boundary
            ...
            double moveDist = (double)nudDistance.Value * 0.5 * mf.inchOrCm2m;
        ...
        private void BtnMakeCurve_Click(object sender, EventArgs e)
        {
            ...
            if (cnt > 3)
            {
                ...
                //create a name
                mf.curve.curveArr[idx].Name = nudDistance.Value.ToString() + " ";
                if (NamenNeu==false) mf.curve.curveArr[idx].Name += (Math.Round(glm.toDegrees(mf.curve.aveLineHeading), 1)).ToString(CultureInfo.InvariantCulture)
        ...
        private void btnCurVor1_Click(object sender, EventArgs e)
        {
            //Kurve in der Reihenfolge um eins nach VOR reihen
            if (mf.curve.numCurveLines > 0)
            {
                int sel = mf.curve.numCurveLineSelected;
                if (sel > 1)
                {
                    mf.curve.curveArr.Insert(sel - 2, mf.curve.curveArr[sel - 1]);
                    mf.curve.curveArr.RemoveAt(sel);
                    mf.curve.numCurveLineSelected--;
                    if (mf.curve.numCurveLineSelected < 1) mf.curve.numCurveLineSelected = 1;
                    FixLabelsCurve();
                }
            }
        }

        private void btnKurNach1_Click(object sender, EventArgs e)
        {
            //Kurve in der Reihenfolge um eins nach HINTEN reihen
            if (mf.curve.numCurveLines > 0)
            {
                int sel = mf.curve.numCurveLineSelected;
                if ((sel > 0) && (sel < mf.curve.numCurveLines))
                {
                    mf.curve.curveArr.Insert(sel + 1, mf.curve.curveArr[sel - 1]);
                    mf.curve.curveArr.RemoveAt(sel - 1);
                    mf.curve.numCurveLineSelected++;
        ...
        private void btnAbVor1_Click(object sender, EventArgs e)
        {
            //AB Linie in der Reihenfolge um eins nach VOR reihen
            if (mf.ABLine.numABLines > 0)
            {
                int sel = mf.ABLine.numABLineSelected;
                if (sel > 1)
                {
                    mf.ABLine.lineArr.Insert(sel - 2, mf.ABLine.lineArr[sel - 1]);
                    mf.ABLine.lineArr.RemoveAt(sel);
                    mf.ABLine.numABLineSelected--;
                    if (mf.ABLine.numABLineSelected < 1) mf.ABLine.numABLineSelected = 1;
                    FixLabelsABLine();
                }
            }
        }

        private void btnAbRueck1_Click(object sender, EventArgs e)
        {
            //AB Linie in der Reihenfolge um eins nach HINTEN reihen
            if (mf.ABLine.numABLines > 0)
            {
                int sel = mf.ABLine.numABLineSelected;
                if ((sel > 0) && (sel < mf.ABLine.numABLines))
                {
                    mf.ABLine.lineArr.Insert(sel + 1, mf.ABLine.lineArr[sel - 1]);
                    mf.ABLine.lineArr.RemoveAt(sel - 1);
                    mf.ABLine.numABLineSelected++;
                    if (mf.ABLine.numABLineSelected > mf.ABLine.numABLines) mf.ABLine.numABLineSelected = mf.curve.numCurveLines;
                    FixLabelsABLine();
                }
            }
        }
        ...
        private void BtnMakeABLine_Click(object sender, EventArgs e)
        {
            ...
            double offset = ((double)nudDistance.Value * 0.5 * mf.inchOrCm2m);
            ...
            //create a name
            mf.ABLine.lineArr[idx].Name = nudDistance.Value.ToString() + " ";
            if (NamenNeu== false) mf.ABLine.lineArr[idx].Name += (Math.Round(glm.toDegrees(mf.ABLine.lineArr[idx].heading), 1)).ToString(CultureInfo.InvariantCulture)
